...
static ngx_conf_post_t ngx_curl_example_enable_post = {
    ngx_curl_example_enable,
};
...
static ngx_command_t ngx_curl_example_commands[] = {
    {
        ...
        0,
        offsetof(ngx_curl_example_conf_t, enable), 
        &ngx_curl_example_enable_post,
    },
    ngx_null_command,
};

static ngx_core_module_t ngx_curl_example_module_ctx = {
    ngx_string("curl_example"), 
    ngx_curl_example_create_conf,
...
static size_t on_read_header(char *data, size_t param, size_t length,
                             void *user_data) {
...
static size_t on_read_body(char *data, size_t param, size_t length, void *user_data) {
  const ngx_curl_allocator_t *allocator = user_data;
  char *buffer = allocator->allocate(length + 1);
  memcpy(buffer, data, length);
  buffer[length] = '\0';
...
static void make_request(ngx_event_t *param) {
...
static ngx_int_t ngx_curl_example_init_process(ngx_cycle_t *param) {
...
static void ngx_curl_example_exit_process(ngx_cycle_t *param) {
