...
static char * ngx_stream_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    *cf = pcf;
    // 初始化 stream phase handlers
    if (ngx_stream_init_phase_handlers(cf, cmcf) != NGX_OK) {
        return NGX_CONF_ERROR;
    }

    // 初始化 &ports，ngx_stream_optimize_servers(cf, &ports) 需要
...
static ngx_int_t ngx_stream_init_phase_handlers(ngx_conf_t *cf, ngx_stream_core_main_conf_t *cmcf) {
    ...
    ngx_stream_phase_handler_t   *phase_handler;
    ...
    phase_handler = ngx_pcalloc(cf->pool, n * sizeof(ngx_stream_phase_handler_t) + sizeof(void *));
    if (phase_handler == NULL) {
        return NGX_ERROR;
    }

    cmcf->phase_engine.handlers = phase_handler;
    n = 0;
    //"NGX_STREAM_PREREAD_PHASE": ngx_stream_ssl_preread_handler(), "NGX_STREAM_CONTENT_PHASE": 
    for (i = 0; i < NGX_STREAM_LOG_PHASE; i++) {
        ...
        case NGX_STREAM_CONTENT_PHASE:
            phase_handler->checker = ngx_stream_core_content_phase;
            n++;
            phase_handler++;
            continue;
        ...
        for (j = cmcf->phases[i].handlers.nelts - 1; j >= 0; j--) {
            ...
            phase_handler->handler = h[j];
            phase_handler->next = n;
            phase_handler++;
        }
    }
...
static ngx_int_t ngx_stream_add_ports(ngx_conf_t *cf, ngx_array_t *ports, ngx_stream_listen_t *listen) {
    ...
    sa = listen->sockaddr;
    p = ngx_inet_get_port(sa); // port=5001
    port = ports->elts;
